...
interface LinkIconProps {
  link: TLink | TLink<ILinkWithTitle>;
  class?: string;
}
...
const LeetCodeIcon: Component<{ title: string }> = props => {
  return (
    <svg
...
const LinkIcon: Component<LinkIconProps> = props => {
  ...
  const isCustomIcon = () => props.link.iconPrefix === EIconLibrary.CUSTOM;
  ...
  return (
    <div class={props.class} title={props.link.iconTitle}>
      ...
      <Show when={isCustomIcon() && props.link.iconName === 'leetcode'}>
        <LeetCodeIcon title={props.link.iconTitle} />
      </Show>
    </div>
  );
};
